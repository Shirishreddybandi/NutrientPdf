...
function load(pdfDocument: string) {
  ...
  // const customizedToolbarItems = PSPDFKit.defaultDocumentEditorToolbarItems.map((item) => {
  //   if (item.type === "remove") {
  //     return {
  //       ...item,
  //       onPress: (documentEditorUIHandler: any) => {
  //         // Your custom delete logic here
  //         console.log("Delete action intercepted");
  //         // If you want to perform the original delete action after your custom logic,
  //         // you can uncomment the next line:
  //         // item.onPress();
  //         const selectedPageIndexes = documentEditorUIHandler.getSelectedPageIndexes();
          
  //         selectedPageIndexes.forEach((pageIndex: any) => {
  //           const overlayElement = document.createElement("div");
  //           overlayElement.className = "PSPDFKit-Note-Annotation-Icon-Cross";
  //           overlayElement.style.cssText = "position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 100px; height: 100px; pointer-events: none;";

  //           const customOverlayItem = new PSPDFKit.CustomOverlayItem({
  //             id: `cross-overlay-${pageIndex}`,
  //             node: overlayElement,
  //             pageIndex: pageIndex,
  ...
  const customizedToolbarItems = PSPDFKit.defaultDocumentEditorToolbarItems.map((item) => {
    if (item.type === "remove") {
      return {
        ...item,
        onPress: (event: Event, documentEditorUIHandler: PSPDFKit.DocumentEditorUIHandler, id: string) => {
          console.log("Delete action intercepted");
          const selectedPageIndexes = documentEditorUIHandler.getSelectedPageIndexes();
          ...
          selectedPageIndexes.forEach((pageIndex: number) => {
            ...
            // Use setOperations to modify the document
            documentEditorUIHandler.setOperations((currentOperations) => {
              // Here you would add your custom logic to add the overlay
              // For now, we're just returning the current operations
  ...
  PSPDFKit.load({
    ...
    documentEditorToolbarItems: customizedToolbarItems,
    ...
    toolbarItems: [
      ...
      },
      {
        ...
        onPress: () => {
          ...
          instance.setViewState((viewState: { zoomIn: () => any }) =>
            viewState.zoomIn()
          );
        },
      },
      {
        ...
        onPress: () => {
          ...
          instance.setViewState((viewState: { zoomOut: () => any }) =>
            viewState.zoomOut()
          );
        },
      },
      ...
      },
      {
        ...
        onPress: () => {
          console.log("Toggling view mode..."); // Get the current view state
          const currentViewState = instance.viewState; // Determine the current scroll mode
          const currentScrollMode = currentViewState.scrollMode; // Toggle the scroll mode
          const newScrollMode =
            currentScrollMode === PSPDFKit.ScrollMode.CONTINUOUS
              ? PSPDFKit.ScrollMode.PER_SPREAD
              : PSPDFKit.ScrollMode.CONTINUOUS; // Keep the current page index and zoom level
          const currentPageIndex = currentViewState.currentPageIndex;
          const zoomLevel = currentViewState.zoomLevel; // Update the view state with the new scroll mode, lock to the current page, and set the zoom level
          const newViewState = currentViewState
            .set("scrollMode", newScrollMode)
            .set("currentPageIndex", currentPageIndex)
            .set("zoomLevel", zoomLevel); // Update the viewer with the new view state
          instance.setViewState(newViewState); // Toggle the button title
          const newTitle =
            newScrollMode === PSPDFKit.ScrollMode.CONTINUOUS
              ? "Toggle View Mode Off"
              : "Toggle View Mode On"; // Update the toolbar item with the new title
    ...
    .then((_instance) => {
      ...
      instance.addEventListener("viewState.zoom.change", () => {
        setTimeout(updateZoomPercentage, 100);
      });
    })
    .catch(console.error);
}
  ...
  textContainer.addEventListener("scroll", () => {
    ...
    if (currentTextElement) {
      ...
      if (currentPageIndexMatch) {
        const currentPageIndex = parseInt(currentPageIndexMatch[1], 10); // Only print if the current page differs from the last visible page
...
function getLastVisibleTextElement(container: HTMLElement) {
  ...
  let firstVisibleElement: HTMLElement = null; // Iterate over all text elements and find the one at the top of the container
  for (let i = 0; i < textElements.length; i++) {
    const textElement = textElements[i];
    const rect = textElement.getBoundingClientRect();
    const containerRect = container.getBoundingClientRect(); // Check if the top of the element is within the visible area of the container
    if (
...
function updateZoomPercentage() {
  const zoom = instance.viewState.zoom; // Get the current zoom
  const currentZoom = instance.currentZoomLevel;
  currentZoomPercentage = Math.round(currentZoom * 100) + "%";
  if (zoom === PSPDFKit.ZoomMode.FIT_TO_WIDTH) {
    currentZoomPercentage = currentZoomPercentage;
  } else if (zoom === PSPDFKit.ZoomMode.FIT_TO_VIEWPORT) {
    currentZoomPercentage = currentZoomPercentage;
  } else if (typeof zoom === "number") {
    currentZoomPercentage = Math.round(zoom * 100) + "%"; // Convert zoom factor to percentage
  } // Update the title of the zoom dropdown button
  // Update the titles for all relevant buttons
  instance.setToolbarItems((prevItems: any[]) =>
    prevItems.map((item) =>
      ["zoom-dropdown-button", "percentage-button"].includes(item.id)
